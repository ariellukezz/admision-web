...
class RevisorPersonalService
{
    public function resumen(int $userId, int $idProceso): array
    {
        ...
        $inscripciones = Inscripcion::where('id_proceso', $idProceso)
            ->where('estado', 0)
            ->where('id_usuario', $userId)
            ->count();

        $inscripcionesHoy = Inscripcion::where('id_proceso', $idProceso)
            ->where('estado', 0)
            ->where('id_usuario', $userId)
            ->whereDate('created_at', $hoy)
            ->count();
    ...
    public function timeline(int $userId, int $idProceso)
    {
        ...
        return $fechas->map(function ($fecha) use ($docs, $comps, $bios) {
            return [
                'fecha' => $fecha,
                'docs'  => $docs->get($fecha)?->cant ?? 0,
                'comps' => $comps->get($fecha)?->cant ?? 0,
                'bios'  => $bios->get($fecha)?->cant ?? 0,
                'total' => ($docs->get($fecha)?->cant ?? 0)
                    + ($comps->get($fecha)?->cant ?? 0)
                    + ($bios->get($fecha)?->cant ?? 0),
            ];
        });
    }

    public function accionesRecientes(int $userId, int $idProceso)
    {
        $docs = DB::table('documento as d')
            ->join('inscripciones as i', 'i.id_postulante', '=', 'd.id_postulante')
        ...
        return $docs->merge($comps)->merge($bios)->merge($inscs)
            ->sortByDesc('fecha')
            ->take(15)
            ->values();
    }

    public function distribucionActividad(int $userId, int $idProceso): array
    {
        $docs = DB::table('documento as d')
            ->join('inscripciones as i', 'i.id_postulante', '=', 'd.id_postulante')
            ->where('i.id_proceso', $idProceso)
        ...
        $inscs = Inscripcion::where('id_proceso', $idProceso)
            ->where('estado', 0)
            ->where('id_usuario', $userId)
            ->count();
    ...
    public function ranking(int $idProceso, int $currentUserId)
    {
        ...
        return $revisores->map(function ($revisor) use ($idProceso, $currentUserId) {
            $docs = DB::table('documento as d')
                ->join('inscripciones as i', 'i.id_postulante', '=', 'd.id_postulante')
                ->where('i.id_proceso', $idProceso)
                ->where('i.estado', 0)
            ...
            return [
                ...
                'es_yo'  => $revisor->id == $currentUserId,
            ];
        })->sortByDesc('total')->values();
    }

    public function pendientes(int $idProceso): array
    {
